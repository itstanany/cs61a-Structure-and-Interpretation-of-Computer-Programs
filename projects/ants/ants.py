    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ...
                assert place.ant is None, 'Two ants in {0}'.format(
                    place)
        Insect.add_to(self, place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        transitions = 0
        # in case of presence of bees in the same place of the ant, return a random bee
        # otherwise: loop through next places in front until you find bees in a nearest place or reaches the beeHive, in this case return None
        if self.place.bees and self.min_range <= transitions <= self.max_range:
            return rANTdom_else_none(self.place.bees)
        else:
            nextPlace = self.place.entrance
            transitions += 1
            while nextPlace and nextPlace != beehive:
                if nextPlace.bees and self.min_range <= transitions <= self.max_range:
                    return rANTdom_else_none(nextPlace.bees)
...
class ShortThrower(ThrowerAnt):
    ...
    food_cost = 2
    # OVERRIDE CLASS ATTRIBUTES HERE
    # overiride max_range attribute to be3
    max_range = 3
    # BEGIN Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # copy of all bees in the place to avoid mutation list that is looped
        bees = self.place.bees[:]

        # damage to all bees in the same place
        for bee in bees:
            Insect.reduce_armor(bee, amount)

        # damage to fireAnt itself
        # the ant is removed if its armor <= 0
        # don't use "Insect" instead of "Ant" to avoid violation of abstraction barriers
        Ant.reduce_armor(self, amount)

        # if fireAnt is killed, cause additional damage to the remaining bees in the same place by amount equal to "damage"
        if not self.place:
            for bee in bees:
                # check for being existance armor in the current place
                # if bee.armor > 0:
                if bee.place:
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        # make and bee armor = 0 and remove it from the place bbes list
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        # in case of not digesting,
        # choose a random bee then eat it and rest digesting counter to required time to digest
        # otherwise: decrease the digesting counter by for each turn passes
        if self.digesting == 0:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen:
            ...
            """Throw a leaf at the nearest Bee in range."""
            self.throw_at(self.nearest_bee(gamestate.beehive))
            nextPlace = self.place.exit
            while nextPlace:
                if nextPlace.ant and (not hasattr(nextPlace.ant, "doubled")):
                    nextPlace.ant.damage *= 2
                    nextPlace.ant.doubled = True
                nextPlace = nextPlace.exit
...
class ContainerAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        # print("inside contain_ant, ant argument", ant)
